...
import { Plus, Search, Filter, MoreHorizontal, User, Calendar as CalendarIcon, CheckCircle2, Circle, Clock } from "lucide-react";
import { useTasks, useCreateTask, useUpdateTask, useDeleteTask } from "@/hooks/use-tasks";
import { useProjects } from "@/hooks/use-projects";
import { useUsers } from "@/hooks/use-users";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { insertTaskSchema, type CreateTaskRequest, type Task } from "@shared/schema";
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <main className="flex-1 md:ml-64 p-8 overflow-y-auto h-screen">
        <header className="flex flex-col md:flex-row md:justify-between md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-display font-bold text-white mb-1">Tasks Board</h1>
            <p className="text-muted-foreground">Track progress across all projects.</p>
          </div>
            ...
            <div className="flex items-center gap-2 bg-card border border-border rounded-xl p-1">
              <button 
                ...
                className={cn(
                  ...
                  filterStatus === "all" ? "bg-primary text-white shadow-sm" : "text-muted-foreground hover:text-white"
                )}
              >
                All
              </button>
              <button 
                ...
                className={cn(
                  "px-3 py-1.5 rounded-lg text-sm font-medium transition-all",
                  filterStatus === "done" ? "bg-primary text-white shadow-sm" : "text-muted-foreground hover:text-white"
                )}
              >
                Completed
              </button>
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-2">
        <span className={cn("text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded", priorityColors[task.priority as keyof typeof priorityColors])}>
          {task.priority}
        </span>
      ...
      <div className="flex items-center justify-between mt-auto">
        <div className="flex -space-x-2">
          {assignee ? (
            <div className="w-6 h-6 rounded-full bg-primary flex items-center justify-center text-[10px] font-bold text-white border-2 border-card" title={assignee.name}>
        ...
        {task.status !== 'done' ? (
          <button 
            onClick={() => updateTask({ id: task.id, status: nextStatus[task.status as keyof typeof nextStatus] })}
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="sm:max-w-[500px] bg-card border-border">
        <DialogHeader>
          <DialogTitle className="text-xl font-display font-bold">New Task</DialogTitle>
        </DialogHeader>
        <Form {...form}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="What needs to be done?" className="bg-background border-border" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Add details..." className="bg-background border-border min-h-[80px]" {...field} value={field.value || ''} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
