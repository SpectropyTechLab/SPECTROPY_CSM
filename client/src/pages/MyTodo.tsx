...
export default function MyTodo() {
  ...
  const updateTaskMutation = useMutation({
    ...
    onMutate: async ({ taskId, updates }) => {
      ...
      queryClient.setQueryData<Task[]>(["/api/tasks/my-todo"], (old = []) =>
        old
          .map((task) => (task.id === taskId ? { ...task, ...updates } : task))
    ...
    onError: (_err, _vars, context) => {
      if (context?.previous) {
        queryClient.setQueryData(["/api/tasks/my-todo"], context.previous);
      }
    },
    onSettled: () => {
  ...
  const handleUpdateTitle = (task: Task, title: string) => {
    updateTaskMutation.mutate({ taskId: task.id, updates: { title } });
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50/80 via-white to-white px-4 py-6 md:px-6 lg:px-8">
      <div className="max-w-[1400px] mx-auto space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">
                A calm, focused view of everything assigned to you.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <section className="flex-1">
            <div className="rounded-3xl bg-white/70 backdrop-blur-sm shadow-sm border border-slate-100">
              ...
              >
                ...
                {filteredTasks.length === 0 && (
                  <div className="px-6 py-12 text-center text-slate-400">
                    <p className="text-sm">{emptyStateCopy[activeFilter]}</p>
                  </div>
                )}
              </motion.div>
            </div>
